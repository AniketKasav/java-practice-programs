...
public class Ass48{
	public static void main(String[] args){
		...
		int temp=num;
		int count=0;
		while(temp>0){        // count number of digit
			temp/=10;
			count++;
		}
		k=k%count;                       // to avoid the unnecessary full rotations that is if 
		int p=(int)Math.pow(10,count-k);  // count is 6 and k=7 then the output is same for the k=7 and k=1 that's why k=k%count
